...
function manifest(): VeilSaveDeploymentManifest {
  ...
  return {
    ...
    strategy: { address: ADDRESS, asset: ADDRESS, id: "test-vault-v1", mode: "TEST_YIELD" as VeilSaveDeploymentManifest["strategy"]["mode"], deploymentBlock: 1 },
    vrf: { confirmations: 3, callbackGasLimit: 100_000, words: 1 },
    pool: { participantCapacity: 16, epochDurationSeconds: 604800, slotBondWei: "1000000000000000", winnerFinalityDelayBlocks: 96, strategyTimelockSeconds: 86400 },
    governance: { safe: ADDRESS, timelock: ADDRESS, guardian: ADDRESS, timelockOpenExecutor: true, timelockSelfAdmin: true },
    evidence: { hcuReportSha256: HASH, aclValidationSha256: HASH, gasReportSha256: HASH, aclValidatedAt: "2026-08-18T00:00:00.000Z" },
  };
}
...
describe("deployment manifest validation", () => {
  ...
  it("rejects runtime bytecode that differs from the manifest", async () => {
    ...
    await expect(verifyManifestCode(publicClient, manifest())).rejects.toThrow(/bytecode does not match/i);
  });
});

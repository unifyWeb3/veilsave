...
import {IVRFV2PlusWrapper} from "../vendor/IVRFV2PlusWrapper.sol";
...
contract MockVrfV2PlusWrapper is IVRFV2PlusWrapper {
    ...
    function estimateRequestPrice(uint32, uint32, uint256) external pure override returns (uint256) {
        return PRICE;
    }

    function estimateRequestPriceNative(uint32, uint32, uint256) external pure override returns (uint256) {
        return PRICE;
    }
    ...
    function fulfill(uint256 requestId, uint256[] calldata randomWords) external {
        IRawVrfConsumer(consumerForRequest[requestId]).rawFulfillRandomWords(requestId, randomWords);
    }
